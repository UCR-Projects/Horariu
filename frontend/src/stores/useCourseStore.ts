...
const useCourseStore = create<CourseState>()(
  persist(
    (set) => ({
      ...
      addGroup: (courseName) => set((state) => {
        const course = state.courses.find(course => course.name === courseName)
        ...
        const newCourses = [...state.courses.filter(course => course.name !== courseName), updatedCourse]

        return {
          courses: newCourses,
          selectedCourse: state.selectedCourse?.name === courseName 
            ? updatedCourse 
            : state.selectedCourse
        }
      }),

      deleteGroup: (courseName, groupName) => set((state) => {
        const course = state.courses.find(course => course.name === courseName)
        if (!course) return state

        const updatedGroups = course.groups.filter(group => group.name !== groupName)
